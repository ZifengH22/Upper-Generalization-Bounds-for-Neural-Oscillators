...
index_rand = randperm(50000, num_sample_select);
%%
...
for i = 1:num_sample_select
    indexl = index_rand(i);
    F_l = Acc_train(:, indexl);
    F_train_all(i,:,:) = F_l;
    X_dX_l = X_train(:,indexl);
    X_dX_output_train_all(i,:,:) = X_dX_l;
    E_X_l = E_X_train(:,indexl);
...
for i = 1:num_case
    ...
    % save([temp_position,'coef_F_',num2str(seed),'.mat'],'coef_F');
    % save([temp_position,'coef_X_output_',num2str(seed),'.mat'],'coef_X_output');
    % save([temp_position,'coef_dX_output_',num2str(seed),'.mat'],'coef_dX_output');
    % save([temp_position,'coef_ddX_output_',num2str(seed),'.mat'],'coef_ddX_output');
    % save([temp_position,'coef_E_X_output_',num2str(seed),'.mat'],'coef_E_X_output');   
end
